...
class App extends Component {
  state = {tabId: tabsList[0].tabId, value: ''}
  ...
  inputChange = e => {
    this.setState(prev => ({tabId: prev.tabId, value: e.target.value}))
  }

  render() {
    const {tabId, value} = this.state
    const apps = appsList.filter(
      obj =>
        obj.category === tabId &&
        obj.appName.toLowerCase().includes(value.toLowerCase()),
    )
    return (
      <div className="body">
